...
class Lead {
  static async create({
    ...
    const hasSellerFields =
      typeof seller_neighborhood !== 'undefined' ||
      typeof seller_size !== 'undefined' ||
      typeof seller_rooms !== 'undefined' ||
      typeof seller_occupancy_status !== 'undefined';
    ...
    const values = [
      ...
      ...(hasSellerFields
        ? [
            ...
            seller_size || null,
            seller_rooms || null,
            seller_occupancy_status || null
          ]
        : [])
    ];

    const placeholders = values.map((_, i) => `$${i + 1}`).join(',');
    const text = `INSERT INTO leads (${columns.join(', ')})
      VALUES (${placeholders})
      RETURNING *`;
